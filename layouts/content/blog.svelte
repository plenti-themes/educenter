        ...
        <div class="bg-white mb-5">
          ...
          {#each allContent.filter(content => content.type == "tags") as tag}
            <li class="list-inline-item mb-2">
              <a href="/educenter/site/tags/advice/">{tag.fields.name}</a>
            </li>
            {/each}
          </ul>
        </div>
                    ...
                    {#each sortByDate(allContent.filter(content => content.type === "posts")) as post,i}
                        {#if i < 3}
                            <div class="media border-bottom border-color pb-3 mb-3">
                                <a href="{post.path}}"><img class="mr-3 post-thumb-sm" src="/media/{post.fields.image.source}" alt="{post.fields.image.alt}"></a>
